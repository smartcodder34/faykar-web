...
function VerifyInner() {
  ...
  const identifier = params.get("email") || params.get("id") || "your email";
    ...
    <div className="min-h-screen w-full px-6 py-10 md:px-10 lg:px-16 flex items-center justify-center">
      <div className="w-full max-w-6xl grid grid-cols-1 md:grid-cols-2 gap-10 items-center">
        <div className="max-w-md">
          ...
          <h1 className="text-4xl md:text-5xl font-semibold text-green-700 tracking-tight">Verify Email</h1>
          <p className="mt-3 text-sm text-gray-600">Enter the 4-digit code we sent to <span className="font-medium">{identifier}</span>.</p>
          ...
          <div className="mt-6 text-sm text-gray-600">
            Didn&apos;t get a code? <button className="text-green-700 hover:underline" onClick={() => console.log("resend code")}>Resend</button>
          </div>
        </div>

        <div className="rounded-3xl overflow-hidden shadow-xl bg-white h-[520px] relative flex items-center justify-center">
          <img alt="verify hero" src="/login-hero.jpg" className="h-full w-full object-cover" />
        </div>
      </div>
    </div>
  );
